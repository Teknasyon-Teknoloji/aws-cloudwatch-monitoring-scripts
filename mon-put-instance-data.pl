...

{
  ...
  $parse_result = GetOptions(
    ...
    'namespace-prefix:s' => \$namespace_prefix,
    'metric-name-prefix:s' => \$metric_name_prefix,
...
if (!defined($namespace_prefix) || length($namespace_prefix) == 0) {
  $namespace_prefix = 'Linux';
}
if (!defined($metric_name_prefix)) {
  $metric_name_prefix = '';
} else {
	$metric_name_prefix .= ' - ';
}
...
$input_ref->{'Namespace'} = "System/" . $namespace_prefix;
...
if ($report_mem_util || $report_mem_used || $report_mem_avail || $report_swap_util || $report_swap_used)
{
  ...
  if ($report_mem_util) {
    ...
    add_metric($metric_name_prefix . 'MemoryUtilization', 'Percent', $mem_util);
  }
  if ($report_mem_used) {
    add_metric($metric_name_prefix . 'MemoryUsed', $mem_units, $mem_used / $mem_unit_div);
  }
  if ($report_mem_avail) {
    add_metric($metric_name_prefix . 'MemoryAvailable', $mem_units, $mem_avail / $mem_unit_div);
  }

  if ($report_swap_util) {
    my $swap_util = 0;
    $swap_util = 100 * $swap_used / $swap_total if ($swap_total > 0);
    add_metric($metric_name_prefix . 'SwapUtilization', 'Percent', $swap_util);
  }
  if ($report_swap_used) {
    add_metric($metric_name_prefix . 'SwapUsed', $mem_units, $swap_used / $mem_unit_div);
  }
}
...
if ($report_disk_space)
{
  ...
  foreach my $line (@df)
  {
    ...
    if ($report_disk_util) {
      ...
      add_metric($metric_name_prefix . 'DiskSpaceUtilization', 'Percent', $disk_util, $fsystem, $mount);
    }
    if ($report_disk_used) {
      add_metric($metric_name_prefix . 'DiskSpaceUsed', $disk_units, $disk_used / $disk_unit_div, $fsystem, $mount);
    }
    if ($report_disk_avail) {
      add_metric($metric_name_prefix . 'DiskSpaceAvailable', $disk_units, $disk_avail / $disk_unit_div, $fsystem, $mount);
    }
  }
}
